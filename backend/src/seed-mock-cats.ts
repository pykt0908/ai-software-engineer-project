...
export async function seedMockCats() {
  ...
  for (const u of MOCK_USERS) {
    ...
    if (loginRes.ok) {
      ...
      if (regRes.ok) {
        const json: any = await regRes.json();
        token = json.jwt;
        userData = json.user;
      } else {
        console.error(`Failed to register user ${u.username}`);
        continue;
      }
    }
  ...
  for (let i = 0; i < POST_TEMPLATES.length; i++) {
    ...
    try {
      ...
      if (postRes.ok) {
        ...
        createdPosts.push(json.data || json);
      }
    } catch (err: any) {}
  }
      ...
      if (i !== j && (i + j) % 2 === 0) {
        ...
        try {
          await fetch(`${API_BASE}/api/users/${target.documentId}/follow`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${follower.jwt}` },
          });
        } catch (e) {}
      }
    }
      ...
      if ((pIdx + uIdx) % 2 === 0 || (pIdx + uIdx) % 3 === 0) {
        ...
        try {
          await fetch(`${API_BASE}/api/posts/${docId}/like`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${likingUser.jwt}` },
          });
        } catch (e) {}
      }
    }
